...
function renderDetail(detailBounty: Bounty = bounty, extraBounties?: Bounty[]) {
  return render(<BountyDetailPage {...detailProps(detailBounty, extraBounties)} />);
}

beforeEach(() => {
  // jsdom does not implement the <dialog> API; mirror DisconnectConfirmModal's setup.
...
describe('BountyDetailPage copy actions', () => {
  ...
  it('shows a QR code for the maintainer wallet address', async () => {
    renderDetail();

    await userEvent.click(
      screen.getByRole('button', { name: /show qr code for maintainer wallet address/i })
    );

    // The address is shown in the metadata row and again inside the modal.
    expect(screen.getAllByText(bounty.maintainer).length).toBeGreaterThanOrEqual(2);
    expect(
      screen.getByRole('img', { name: `QR code for wallet address ${bounty.maintainer}` })
    ).toBeInTheDocument();

    // Close the modal and verify it is removed.
    await userEvent.click(screen.getByRole('button', { name: 'Close' }));
    expect(
      screen.queryByRole('img', { name: `QR code for wallet address ${bounty.maintainer}` })
