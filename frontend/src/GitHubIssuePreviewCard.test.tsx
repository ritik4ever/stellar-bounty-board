...
describe('GitHubIssuePreviewCard', () => {
  ...
  it('fetches live issue data and renders the title and labels', async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(githubIssueApiUrl('ritik4ever/stellar-bounty-board', 287), {
  ...
  it('shows an error state with a GitHub link when the fetch fails', async () => {
    ...
    // The whole card is a link in the error state too, so scope the query to the
    // alert region to find the explicit error link.
    const link = within(screen.getByRole('alert')).getByRole('link', {
      name: /view issue on github/i,
    });
  ...
  it('shows a rate-limit note when GitHub returns 403', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/unauthenticated api requests are rate-limited/i)
      ).toBeInTheDocument();
    });
  });
