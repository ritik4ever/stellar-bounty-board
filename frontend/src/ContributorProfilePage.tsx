...
export default function ContributorProfilePage({
  ...
  useEffect(() => {
    ...
    setMeta(
      'description',
      `Profile for contributor ${shortAddress(address)} — earned reputation and completed work.`
    );
  ...
  const stats = useMemo(() => {
    ...
    const totalEarned = released.reduce(
      (s, b) => s + (typeof b.amount === 'number' ? b.amount : 0),
      0
    );
    const completed = released.length;
    const activeReservations = reserved.length;
    const disputeRate =
      completed + refunded.length > 0
        ? Math.round((refunded.length / (completed + refunded.length)) * 100)
        : 0;
        ...
        {stats.released.length === 0 ? (
          ...
          <ul className="completed-list">
            {stats.released.map((b) => (
              <li key={b.id}>
                <a
                  href={`https://github.com/${b.repo}/issues/${b.issueNumber}`}
                  target="_blank"
                  rel="noreferrer"
                >
          ...
          <ol>
            {leaderboard.slice(0, 5).map((entry) => (
              <li key={entry.address}>
                {entry.address} — {entry.totalXlm} XLM
              </li>
            ))}
          </ol>
        )}
