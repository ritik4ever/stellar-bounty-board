...
import { FolderGit2, Kanban, LayoutList, Moon, Rocket, Search, Sun } from 'lucide-react';
...
import KanbanBoard, { type KanbanAction } from './KanbanBoard';
...
function App() {
  ...
  /** "list" = existing card list view; "board" = kanban board */
  ...
  /**
   * Called by KanbanBoard when a card is dropped onto a valid target column.
   * Delegates to the same handlers used by the list view so logic is not
   * duplicated.
   */
  const handleKanbanTransition = useCallback(
    async (bounty: Bounty, action: KanbanAction) => {
      if (action === 'reserve') {
        await handleReserve(bounty);
      } else if (action === 'submit') {
        await handleSubmit(bounty);
      } else if (action === 'release') {
        await handleRelease(bounty);
      } else if (action === 'refund') {
        await handleRefund(bounty);
      }
    },
      ...
      <main className="main-content">
        ...
        <section className="board-section">
          <div className="board-filters">
            ...
            <div className="view-toggle" role="group" aria-label="Switch board view">
              <button
                type="button"
              ...
              <button
                type="button"
                className={`view-toggle__btn ${viewMode === 'board' ? 'active' : ''}`}
                title="Board view"
