...
const defaultTemplates: BountyTemplate[] = [];
...
function App() {
  ...
  const [templates, setTemplates] = useState<BountyTemplate[]>(defaultTemplates);

  useBeforeUnload(isFormDirty);

  useEffect(() => {
    const controller = new AbortController();
    listBountyTemplates(controller.signal)
      .then(setTemplates)
      .catch(() => {
        // Silently handle — picker will remain hidden when no templates exist
      });
