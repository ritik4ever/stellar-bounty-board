...
registry.registerPath({
  ...
  description:
    'Returns the service name and current server timestamp. Use this to verify the API is reachable.',
  responses: {
...
registry.registerPath({
  ...
  responses: {
    200: jsonResponse(
      'Array of all bounty records.',
      z.object({ data: z.array(bountyRecordSchema) }),
    ),
  },
});
...
registry.registerPath({
  ...
  responses: {
    ...
    400: errorResponse(
      'Bounty not found, not reserved, contributor mismatch, or validation failed.',
    ),
  },
});
...
registry.registerPath({
  ...
  responses: {
    ...
    400: errorResponse(
      'Bounty not found, not submitted, maintainer mismatch, or validation failed.',
    ),
  },
});
...
registry.registerPath({
  ...
  responses: {
    ...
    400: errorResponse(
      'Bounty not found, already finalised, maintainer mismatch, or validation failed.',
    ),
  },
});
...
const leaderboardEntrySchema = z
  .object({
    address: z
      .string()
      .openapi({ example: 'GBBB...BBB', description: 'Contributor Stellar address.' }),
    totalXlm: z
      .number()
      .openapi({ example: 350.5, description: 'Total XLM received from released bounties.' }),
    bountiesCompleted: z
      .number()
      .int()
      .openapi({ example: 3, description: 'Number of released bounties completed.' }),
  })
