...
export function createStellarSignatureAuthMiddleware(
  options: StellarSignatureAuthOptions = {}
): RequestHandler {
  const {
  ...
  return (req, res, next) => {
    ...
    if (requireConfiguredPublicKey && allowedPublicKeys.length === 0) {
    ...
    const bodyPublicKey =
      typeof req.body?.[bodyPublicKeyField] === 'string' ? req.body[bodyPublicKeyField] : undefined;
    if (bodyPublicKey && bodyPublicKey !== publicKeyHeader) {
      res
        .status(401)
        .json({ error: `Request ${bodyPublicKeyField} does not match signer public key.` });
      return;
    }
