...
/**
 * Global-lock wrapper (same implementation as withStoreLock — provided as a
 * named export for functions whose JSDoc documents "global lock" semantics).
 */
async function withGlobalLock<T>(fn: () => T | Promise<T>): Promise<T> {
...
/**
 * Global platform metrics computed across all bounties.
 */
export interface GlobalMetrics {
  totalBounties: number;
  totalReleased: number;
...
/**
 * Compute aggregate platform-wide metrics synchronously.
 */
export function getGlobalMetrics(): GlobalMetrics {
  const bounties = listBounties();
  const released = bounties.filter((b) => b.status === "released");
  const disputed = bounties.filter((b) => b.status === "disputed");
  const contributors = new Set(
    released.map((b) => b.contributor).filter(Boolean),
  );
  const totalVolumeXlm = released.reduce((sum, b) => sum + b.amount, 0);
...
/**
 * Compute metrics for a specific maintainer.
 */
export function getMaintainerMetrics(maintainer: string): MaintainerMetrics {
  const bounties = listBounties().filter((b) => b.maintainer === maintainer);
  const released = bounties.filter((b) => b.status === "released");
...
/**
 * Options for listing all audit logs across all bounties.
 */
export interface ListAllAuditLogsOptions {
  limit: number;
  offset: number;
...
/**
 * List audit log records across all bounties with optional filters.
 */
