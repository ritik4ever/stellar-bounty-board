...
describe("PR URL repo validation", () => {
  it("accepts a GitHub PR for the bounty repo", async () => {
    // In test environment with mocking, valid format + matching repo should succeed
    await expect(validateGithubPrUrlForRepo("https://github.com/owner/repo/pull/123", "owner/repo")).resolves.not.toThrow();
  ...
  it("accepts private GitHub repository URL patterns when the repo matches", async () => {
    // In test environment with mocking, valid format + matching repo should succeed
    await expect(validateGithubPrUrlForRepo("https://github.com/private-org/private-repo/pull/7", "private-org/private-repo")).resolves.not.toThrow();
  });
});
