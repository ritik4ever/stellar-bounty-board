...
describe('bountyStore — expiration via normalizeRecords', () => {
  ...
  it('marks reserved bounties past deadline as expired when listed', async () => {
    ...
    await expect(async () => await reserveBounty('BNT-0001', CONTRIBUTOR)).rejects.toThrow(
      /only open/i,
    );
  });
});
...
describe('bountyStore — invalid transitions and errors', () => {
  it('throws when bounty id is missing', async () => {
    ...
    await expect(async () => await reserveBounty('BNT-9999', CONTRIBUTOR)).rejects.toThrow(
      /not found/i,
    );
  });
  ...
  it('submit: requires reserved and matching contributor', async () => {
    ...
    await expect(
      async () => await submitBounty(b.id, CONTRIBUTOR, 'https://example.com/pr/1'),
    ).rejects.toThrow(/only reserved/i);

    await reserveBounty(b.id, CONTRIBUTOR);
    await expect(
      async () => await submitBounty(b.id, OTHER_ACCOUNT, 'https://example.com/pr/1'),
    ).rejects.toThrow(/reserved contributor/i);
  });
  ...
  it('release: requires maintainer and submitted status', async () => {
    ...
    await expect(async () => await releaseBounty(b.id, MAINTAINER)).rejects.toThrow(
      /only submitted/i,
    );
    ...
    await submitBounty(b.id, CONTRIBUTOR, 'https://example.com/pr/1');
    await expect(async () => await releaseBounty(b.id, OTHER_ACCOUNT)).rejects.toThrow(
  ...
  it('refund: rejects wrong maintainer, submitted, and finalized', async () => {
    ...
    await expect(async () => await refundBounty(openB.id, OTHER_ACCOUNT)).rejects.toThrow(
      /maintainer address/i,
    );
    ...
    await expect(async () => await refundBounty(flow.id, MAINTAINER)).rejects.toThrow(
      /submitted bounties/i,
    );
...
describe('bountyStore — event history and metrics', () => {
  ...
  it('getMaintainerMetrics returns accurate counts', async () => {
    const { createBounty, reserveBounty, submitBounty, releaseBounty, getMaintainerMetrics } =
      await loadStore();
  ...
  it('race condition prevention: version mismatch on reserve', async () => {
    ...
    await expect(async () => await reserveBounty(bounty.id, OTHER_ACCOUNT, 1)).rejects.toThrow(
