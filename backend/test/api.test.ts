...
describe('GET /api/leaderboard', () => {
  ...
  it('aggregates multiple released bounties for the same contributor', async () => {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      await request(app)
        ...
        .send({
          contributor: CONTRIBUTOR,
          submissionUrl: `https://github.com/owner/repo/pull/${i + 1}`,
        })
        .expect(200);
      await request(app)
        .post(`/api/bounties/${id}/release`)
  ...
  it('ranks higher XLM earner first', async () => {
    ...
    await request(app)
      .post(`/api/bounties/${c1.data.id}/reserve`)
      .send({ contributor: CONTRIBUTOR })
      .expect(200);
    await request(app)
      .post(`/api/bounties/${c1.data.id}/submit`)
      .send({ contributor: CONTRIBUTOR, submissionUrl: 'https://github.com/o/r/pull/1' })
      .expect(200);
    await request(app)
      .post(`/api/bounties/${c1.data.id}/release`)
      .send({ maintainer: MAINTAINER })
      .expect(200);
    ...
    for (let i = 0; i < 2; i++) {
      const { body: c2 } = await request(app)
        ...
        .expect(201);
      await request(app)
        .post(`/api/bounties/${c2.data.id}/reserve`)
        .send({ contributor: OTHER_ACCOUNT })
        .expect(200);
      await request(app)
        .post(`/api/bounties/${c2.data.id}/submit`)
        .send({
          contributor: OTHER_ACCOUNT,
          submissionUrl: `https://github.com/o/r/pull/${i + 10}`,
        })
        .expect(200);
      await request(app)
        .post(`/api/bounties/${c2.data.id}/release`)
        .send({ maintainer: MAINTAINER })
        .expect(200);
    }
