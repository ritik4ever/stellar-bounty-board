...
use soroban_sdk::{IntoVal, testutils::Events,
...
fn test_fee_stats_release_and_dispute_release_paths() {
    ...
    let (client, _admin, maintainer, contributor, token_id, fee_recipient, arbiter) = setup_test(&env);
    let token = TokenClient::new(&env, &token_id);
    let token_admin = soroban_sdk::token::StellarAssetClient::new(&env, &token_id);
    token_admin.mint(&maintainer, &10_000);
