...
fn test_create_bounty() {
    ...
    let (client, _admin, maintainer, _contributor, token_id, _fee_recipient, _arbiter) =
        setup_test(&env);
...
fn test_full_lifecycle() {
    ...
    let (client, _admin, maintainer, contributor, token_id, _fee_recipient, _arbiter) =
        setup_test(&env);
    let token = TokenClient::new(&env, &token_id);
    let token_admin = soroban_sdk::token::StellarAssetClient::new(&env, &token_id);
    token_admin.mint(&maintainer, &1000);
...
fn test_concurrent_reserve_two_contributors() {
    ...
    assert_eq!(
        bounty.status,
        BountyStatus::Reserved,
        "First reserve should transition to Reserved"
    );
    assert_eq!(
        bounty.contributor,
...
fn test_arbiter_rotation_success() {
    ...
    let (client, _admin, admin, _, _, _, _) = setup_test(&env);
    let new_arbiter = Address::generate(&env);

    client.set_arbiter(&new_arbiter);

    env.ledger()
        .set_timestamp(env.ledger().timestamp() + 86400 * 2 + 1);
...
fn test_create_bounty_with_custom_dispute_window_override() {
    ...
    assert_eq!(
        client.get_effective_dispute_window(&bounty_id),
        custom_window
    );
}
...
// ─── Partial-Release Dispute Resolution Tests (Issue #726) ──────────────

/// Helper: create a bounty, submit it, dispute it, and advance time past the
/// dispute window so `resolve_dispute` can be called.
fn setup_disputed_bounty(
    env: &Env,
    client: &StellarBountyBoardContractClient<'static>,
    maintainer: &Address,
    contributor: &Address,
    ...
    // Advance time past the dispute window (600s default)
    env.ledger().set_timestamp(env.ledger().timestamp() + 601);

    bounty_id
}

/// 50/50 split with zero fees: each party gets 250 of the 500 escrow.
#[test]
fn test_resolve_dispute_50_50_split_no_fee() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    let bounty_id = setup_disputed_bounty(
        &env,
        &client,
        &maintainer,
        &contributor,
    ...
    client.resolve_dispute(&bounty_id, &250, &250);

    let bounty = client.get_bounty(&bounty_id);
    assert_eq!(bounty.status, BountyStatus::Released);
    // maintainer was minted 1_000_000, sent 500 to escrow, gets 250 back = 999_750
    assert_eq!(token.balance(&maintainer), 999_750);
    assert_eq!(token.balance(&contributor), 250);
}

/// Full release via split: (amount, 0) gives all to contributor minus fee.
#[test]
fn test_resolve_dispute_full_release_via_split() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    // 100% to contributor, 0% to maintainer → full release
    client.resolve_dispute(&bounty_id, &500, &0);

    let bounty = client.get_bounty(&bounty_id);
    assert_eq!(bounty.status, BountyStatus::Released);
    // maintainer was minted 1_000_000, sent 500 to escrow, gets 0 back = 999_500
    assert_eq!(token.balance(&maintainer), 999_500);
    assert_eq!(token.balance(&contributor), 500);
}

/// Full refund via split: (0, amount) gives all to maintainer.
#[test]
fn test_resolve_dispute_full_refund_via_split() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    // 0% to contributor, 100% to maintainer → full refund
    client.resolve_dispute(&bounty_id, &0, &500);

    let bounty = client.get_bounty(&bounty_id);
    assert_eq!(bounty.status, BountyStatus::Refunded);
    // maintainer was minted 1_000_000, sent 500 to escrow, gets 500 back = 1_000_000
    assert_eq!(token.balance(&maintainer), 1_000_000);
    assert_eq!(token.balance(&contributor), 0);
}

/// Split that doesn't sum to escrowed total is rejected.
#[test]
#[should_panic(expected = "SplitAmountMismatch")]
fn test_resolve_dispute_split_amount_mismatch() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    // 300 + 200 = 500 ✓  but we pass 300 + 100 = 400 ✗
    client.resolve_dispute(&bounty_id, &300, &100);
}

/// 50/50 split with 10% (1000 bps) fee: fee is deducted only from
/// the contributor's 250 → contributor_net = 250 - 25 = 225.
#[test]
fn test_resolve_dispute_50_50_split_with_fee() {
    let env = Env::default();
    env.mock_all_auths();

    let (client, _admin, maintainer, contributor, token_id, fee_recipient, _arbiter) =
        setup_test(&env);
    let token = TokenClient::new(&env, &token_id);
    ...
    // contributor receives 250 - floor(250 * 1000 / 10_000) = 250 - 25 = 225
    assert_eq!(token.balance(&contributor), 225);
    // fee_recipient gets 25
...
/// Fee is proportional to contributor share only. With 70/30 split and 10% fee:
/// contributor gets 350 - 35 = 315, maintainer gets 150, fee = 35.
#[test]
fn test_resolve_dispute_70_30_split_proportional_fee() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    // 70% contributor, 30% maintainer
    client.resolve_dispute(&bounty_id, &350, &150);

    // contributor: 350 - floor(350 * 1000 / 10_000) = 350 - 35 = 315
    assert_eq!(token.balance(&contributor), 315);
    assert_eq!(token.balance(&fee_recipient), 35);
    // maintainer was minted 1_000_000, sent 500 to escrow, gets 150 back = 999_650
    assert_eq!(token.balance(&maintainer), 999_650);
}

/// Negative split amounts are rejected.
#[test]
#[should_panic(expected = "InvalidAmount")]
fn test_resolve_dispute_negative_amount_rejected() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    client.resolve_dispute(&bounty_id, &600, &-100);
}

/// resolve_dispute requires arbiter auth. With mock_all_auths all auth
/// checks pass, so we verify the function completes successfully.
#[test]
...
fn test_get_bounties_by_contributor_empty() {
    let env = Env::default();
    let (client, _, _, contributor, _, _, _) = setup_test(&env);
...
fn test_get_bounties_by_contributor_no_match() {
    ...
    assert_eq!(
        result.len(),
        0,
        "open bounties should not appear for any contributor"
    );
}
...
fn test_get_bounties_by_contributor_multiple_bounties() {
    ...
    assert_eq!(
        result.len(),
        2,
        "only the two bounties belonging to our contributor should be returned"
    );
    assert_eq!(result.get(0).unwrap().issue_number, 1);
    assert_eq!(result.get(0).unwrap().status, BountyStatus::Reserved);
...
fn test_get_bounties_by_contributor_includes_released() {
    ...
    assert_eq!(
        result.len(),
        1,
        "released bounty should appear in contributor history"
    );
